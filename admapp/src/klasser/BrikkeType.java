...
public enum BrikkeType{
    ...
    public static ArrayList<String> getTyper(){
        BrikkeType[] typer = BrikkeType.values();
        ArrayList<String> typerString = new ArrayList<>(BrikkeType.values().length);
        for (int i=0; i<typer.length; i++){
            typerString.add(typer[i].toString());
        }
        return typerString;
    }
